...
cdef extern from "ibpsolver.h" namespace "george":

    cdef cppclass IbpSolver:
        IbpSolver(unsigned int, unsigned int, unsigned int, double)
        int compute (const double*, const double*, unsigned int)
...
def _rebuild(row, col, nleaf, tol):
    return IbpHODLRSolver(row=row, col=col, nleaf=nleaf, tol=tol)


cdef class IbpHODLRSolver:
    """
    ...
    cdef IbpSolver* solver
    cdef unsigned int nleaf
    cdef double tol

    def __cinit__(self, row, col, unsigned int nleaf=100, double tol=1e-12):
        self.row = row
        self.col = col
        self.solver = new IbpSolver(row, col, nleaf, tol)
        self.nleaf = nleaf
        self.tol = tol

    def __reduce__(self):
        return _rebuild, (self.row, self.col, self.nleaf, self.tol)
    ...
    def compute(self, np.ndarray[DTYPE_t, ndim=2] x,
        ...
        # Compute the matrix.
        cdef int info
        info = self.solver.compute(<double*>x.data, <double*>yerr.data, seed)
        if info != 0:
