...
cdef extern from "latentsolver.h" namespace "george":

    cdef cppclass LatentSolver:
        LatentSolver(kernels.LatentModelKernel*, unsigned int, double)
...
cdef class LatentHODLRSolver:
    ...
    :param kernel:
        A subclass of :class:`LatentModelKernel` implementing a kernel function.
    ...
    cdef kernels.LatentModelKernel* kernel
    cdef LatentSolver* solver
    cdef unsigned int nleaf
    cdef double tol

    def __cinit__(self, kernel_spec, unsigned int nleaf=100, double tol=1e-12):
        self.kernel_spec = kernel_spec
        self.kernel = <kernels.LatentModelKernel *>kernels.parse_kernel(kernel_spec)
        self.solver = new LatentSolver(self.kernel, nleaf, tol)
        self.nleaf = nleaf
        self.tol = tol
